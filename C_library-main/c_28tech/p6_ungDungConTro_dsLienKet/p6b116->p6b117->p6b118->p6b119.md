# Sắp xếp danh sách liên kết
## Công thức
```
void sort(node **head) {
  for (node *i = *head; i != NULL; ii->next){
    node *min = i;
      for (node *j = i->next; j != NULL; j = j->next){
          if(j->data < min->data) {
              min = j;
          }
      }
      int tmp = min->data;
      min->data i->data;  //Thuật toán sử dụng: Selection Sort
      i->data = tmp;
  }
}
// Chú ý là ta chỉ cần hoán vị phần data của 2 node còn không cần phải hoán vị next.
```
## Ví dụ
```
/******************************************************************************

                            Online C Compiler.
                Code, Compile, Run and Debug C program online.
Write your code in this editor and press "Run" button to compile and execute it.

*******************************************************************************/

#include <stdio.h>
#include <string.h>
#include <stdlib.h>

struct node {
	int data;
	struct node *next;//link
};

typedef struct node node;

void duyet(node *head) {
	while(head != NULL) {
		printf("%d ", head->data);
		//dich head sang node ben phai
		head = head->next;//quan trong
	}
	printf("\n");
}

int size(node *head) {
	int cnt = 0;
	while(head != NULL) {
		++cnt;
		head = head->next;
	}
	return cnt;
}

node* makeNode(int x) {
	//Cap phat dong 1 node
	node *newNode = (node*)malloc(sizeof(node));
	newNode->data = x;
	newNode->next = NULL;
	return newNode;
}

//themDau
void pushFront(node **head, int x) {
	node *newNode = makeNode(x);
	newNode->next = (*head); // toan tu giai tham chieu
	(*head) = newNode;
}

//themCuoi
void pushBack(node **head, int x) {
	node *newNode = makeNode(x);
	node *temp = (*head);
	if(temp == NULL){
	    (*head) = newNode; return;
	}
	while(temp->next != NULL) {
		temp = temp->next;
	}
	temp->next = newNode;
}

// chen vao vi tri k trong danh sach lien ket
void insert(node ** head, int k, int x){
    int n = size(*head);
    if(k < 1 || k > n + 1){
        printf("Vi tri chen khong hop le!\n");
        return;
    }
    if(k == 1){
        pushFront(head, x);
    }
    else if(k == n + 1){
        pushBack(head, x);
    }
    else{
        node *temp = (*head);
        for(int i = 1; i < k - 1; i++){
            temp = temp->next;
        }
        node *newNode = makeNode(x);
        //Buoc 1: phan next cua new Node luu dia chi thu k
        newNode->next = temp->next;
        //Buoc 2: phan next cua node thu k - 1 luu dia chi cua node moi
        temp->next = newNode;
    }
}


//xoaDau
void popFront(node **head){
    //rong thi thoat khong dung
    if(*head == NULL) return;
    node *temp = *head; // free
    *head = (*head)->next;
    // hoac *head = tmp->next;
    free(temp);
}

//xoaCuoi
void popBack(node **head){
    //rong thi thoat khong dung
    if(*head == NULL) return;
    node *temp = (*head);
    if(temp->next == NULL){
        *head = NULL; free(temp); return;
    }
    while(temp->next->next != NULL){
        temp = temp->next;
    }
    // luu node cuoi cung
    node *last = temp->next;
    temp-> next = NULL;
    free(last);
}

//xoa o vi tri k trong danh sach lien ket
void erase(node **head, int k){
    if(k < 1 || k > size(*head)){
        return;
    }
    if(k == 1){
        popFront(head);
    }
    else{
        node *temp = (*head);
        for(int i = 1; i < k - 1; i++){
            temp = temp->next;
        }
        //temp: k - 1
        node* del = temp->next; // node thu k
        temp->next = del->next;
        free(del);
        
    }
}


void sort(node **head){
    for(node *i = (*head); i != NULL; i = i->next){
        node *min = i;
        for(node *j = i->next; j != NULL; j = j->next){
            if(min->data > j->data){
                min = j;
            }
        }
        //hoan vi
        int tmp = min->data;
        min->data = i->data;
        i->data = tmp;
    }
}


int main() {
	node *head = NULL;
	while(1) {
		printf("---------------------------------\n");
		printf("1. Them vao cuoi\n");
		printf("2. Them vao dau\n");
		printf("3. Chen vao giua\n");
		printf("4. Xoa dau\n");
		printf("5. Xoa giua\n");
		printf("6. Xoa cuoi\n");
		printf("7. Sap xep DSLK!\n");
		printf("8. Duyet DSLK\n");
		printf("0. Thoat !\n");
		printf("---------------------------------\n");
        printf("Nhap lua chon: ");
        int lc; scanf("%d", &lc);
        if(lc == 1){
            printf("Nhap gia tri can them: ");
            int x; scanf("%d", &x);
            pushBack(&head, x);
        }
        else if(lc == 2){
            printf("Nhap gia tri can them: ");
            int x; scanf("%d", &x);
            pushFront(&head, x);
        }
        else if(lc == 3){
            printf("Nhap vi tri chen: ");
            int k; scanf("%d", &k);
            printf("Nhap gia tri can chen: ");
            int x; scanf("%d", &x);
            insert(&head, k, x);
        }
        else if(lc == 4){
            popFront(&head);
        }
        else if(lc == 5){
            printf("Nhap vi tri xoa: ");
            int k; scanf("%d", &k);
            erase(&head, k);
        }
        else if(lc == 6){
            popBack(&head);
        }
        else if(lc == 7){
            sort(&head);
        }
        else if(lc == 8){
            duyet(head);
        }
        else if(lc == 0){
            break;
        }
	}
	duyet(head);
	return 0;
}
```

# DSLK đôi (Double Linked List):
```
/******************************************************************************

                            Online C Compiler.
                Code, Compile, Run and Debug C program online.
Write your code in this editor and press "Run" button to compile and execute it.

*******************************************************************************/

#include <stdio.h>
#include <string.h>
#include <stdlib.h>

struct node {
	int data;
	struct node *next; // node truoc
	struct node *prev; // node sau
};


typedef struct node node;


node* makeNode(int x){
    node *newNode = (node*)malloc(sizeof(node));
    newNode->data = x;
    newNode->next = NULL;
    newNode->prev = NULL;
    return newNode;
}


// duyet dslk
void duyet(node *head){
    while(head != NULL){
        printf("%d", head->data);
        head = head->next;
    }
}


// themDau
void pushFront(node **head, int x){
    node *newNode = makeNode(x);
    newNode->next = (*head);
    if(*head != NULL){
        (*head)->prev = newNode;
    }
    (*head) = newNode;
}


// themCuoi
void pushBack(node **head, int x){
    node *newNode = makeNode(x);
    if(*head == NULL){
        *head = newNode; return;
    }
    node *temp = *head;
    while(temp->next != NULL){
        temp = temp->next;
    }
    temp->next = newNode;
    newNode->prev = temp;
}


//them vao node thu k trong dslk
void insert(node **head, int k, int x){
    int size(node *head);
    int n = size(*head);
    if(k < 1 || k > n + 1){
        printf("Chen khong hop le!\n");
        return;
    }
    if(k == 1){
        pushFront(head, x);
    }
    else if(k == n + 1){
        pushBack(head, x);
    }
    else {
        node *temp = (*head);
        for(int i = 1; i < k - 1; i++){
            temp = temp->next;
        }
        // temp : node thu k - 1
        node *newNode = makeNode(x);
        newNode->next = temp->next;
        temp->next->prev = newNode;
        temp->next = newNode;
        newNode->prev = temp;
    }
}


// xoaDau
void popFront(node ** head){
    if(*head == NULL){
        printf("Xoa node khong hop le(*head == NULL)!\n");
        return;
    }
    node *temp = *head;
    *head = temp->next;
    if(*head != NULL){
        (*head)->prev = NULL;
    }
    free(temp);
}


// xoaCuoi
void popBack(node ** head){
    if(*head == NULL){
        printf("Xoa node khong hop le(*head == NULL)!\n");
        return;
    }
    node *temp = *head;
    if(temp->next == NULL) {
        *head = NULL; free(temp);
        return;
    }
    while(temp->next != NULL){
        temp = temp->next;
    }
    temp->prev->next = NULL;
    free(temp);
    // node *last = temp->next; // node cuoi
    // temp->next = NULL;
    // free(last);
}


// xoaGiua
void erase(node **head, int k){
    int size(node *head);
    int n = size(*head);
    if(k < 1 || k > n){
        printf("Xoa chen khong hop le!\n");
        return;
    }
    if (k == 1){
        popFront(head);
    }
    else {
        node *temp = *head;
        for(int i = 1; i <= k - 1; i++){
            temp = temp->next;
        }
        temp->prev->next = temp->next; // k - 1 => k + 1 : next
        if(temp->next != NULL){
            temp->next->prev = temp->prev; // k + 1 => k - 1 : prev
        }
        free(temp);
    }
}


//dem so node
int size(node *head){
    int cnt = 0;
    while(head != NULL){
        ++cnt;
        head = head ->next;
    }
    return cnt;
}


//duyetNguoc
void duyetNguoc(node *head){
    if(head == NULL){
        return;
    }
    while(head->next != NULL){
        head = head->next;
    }
    while(head != NULL){
        printf("%d", head->next);
        head = head->prev;
    }
}


//sapXepDSLKDoi
void sort(node **head){
    for(node *i = *head; i != NULL; i = i->next){
        node *min = i;
        for(node *j = i->next; j != NULL; j = j->next){
            if(min->data > j->data){
                min = j;
            }
        }
        int temp = min->data;
        min->data = i->data;
        i->data = temp;
    }
}


int main() {
	node *head = NULL;
	while(1) {
		printf("\n------Double Linked List Operation------\n");
		printf("1. Them vao cuoi\n");
		printf("2. Them vao dau\n");
		printf("3. Chen vao giua\n");
		printf("4. Xoa dau\n");
		printf("5. Xoa giua\n");
		printf("6. Xoa cuoi\n");
		printf("7. Duyet DSLK\n");
		printf("8. Duyet nguoc DSLK\n");
		printf("9. Sap xep\n");
		printf("0. Thoat !\n");
		printf("----------------------------------------\n");
        printf("Nhap lua chon: ");
        int lc; scanf("%d", &lc);
        if(lc == 1){
            printf("Nhap gia tri can them: ");
            int x; scanf("%d", &x);
            pushBack(&head, x);
        }
        else if(lc == 2){
            printf("Nhap gia tri can them: ");
            int x; scanf("%d", &x);
            pushFront(&head, x);
        }
        else if(lc == 3){
            printf("Nhap vi tri chen: ");
            int k; scanf("%d", &k);
            printf("Nhap gia tri can chen: ");
            int x; scanf("%d", &x);
            insert(&head, k, x);
        }
        else if(lc == 4){
            popFront(&head);
        }
        else if(lc == 5){
            printf("Nhap vi tri xoa: ");
            int k; scanf("%d", &k);
            erase(&head, k);
        }
        else if(lc == 6){
            popBack(&head);
        }
        else if(lc == 7){
            duyet(head);
        }
        else if(lc == 8){
            duyetNguoc(head);
        }
        else if(lc == 9){
            sort(&head);
        }
        else if(lc == 0){
            break;
        }
	}
	duyet(head);
	return 0;
}
```
